...
$csvs = glob(__DIR__ . '/storage/scans/scan_rede_*.csv');
  ...
  fetch('api_ferramentas.php?action=scan_status')
    ...
    .then(text => {
      ...
      try { d = JSON.parse(text); } catch(e) {
        console.error('scan_poll_json_err', e.message, text.slice(0,120));
        return; // keep polling
      }
      return d;
    })
    .then(d => {
      ...
      const logText = d.log_b64 ? new TextDecoder().decode(Uint8Array.from(atob(d.log_b64), c => c.charCodeAt(0))) : (d.log || '');
      if (logText) { log.textContent = logText; log.scrollTop = log.scrollHeight; }
