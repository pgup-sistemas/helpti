...
$pdo        = db();
$u          = usuario();
$tecnicos   = $pdo->query("SELECT id,nome FROM usuarios WHERE ativo=1 AND perfil IN ('tecnico','admin') AND deleted_at IS NULL ORDER BY nome")->fetchAll();
$categorias = $pdo->query("SELECT id,nome,icone FROM categorias WHERE ativo=1 ORDER BY nome")->fetchAll();
$erros      = [];

// Unidades ativas e unidade padrão do usuário logado
$unidades = $pdo->query("SELECT id, nome, codigo, sla_multiplicador FROM unidades WHERE ativo=1 ORDER BY nome")->fetchAll();
$st_unid  = $pdo->prepare("SELECT unidade_id FROM usuarios_unidades WHERE usuario_id=? AND eh_padrao=1 LIMIT 1");
$st_unid->execute([$u['id']]);
$unidade_padrao_id = (int)($st_unid->fetchColumn() ?: (count($unidades) === 1 ? $unidades[0]['id'] : 0));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $unid_id   = (int)($_POST['unidade_id'] ?? 0) ?: null;
    ...
    if (!$erros) {
        ...
        $pdo->prepare("INSERT INTO chamados (numero,descricao,setor,solicitante,responsavel_id,nivel,categoria_id,status,semana,origem,inventario_id,imagens,unidade_id)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)")
            ->execute([$numero,$desc,$setor,$nome,$resp,$nivel,$cat_id,$status,$semana,$origem,$inv_id,$imagens_json,$unid_id]);
  ...
  <div class="card-body">
    ...
    <form method="post" enctype="multipart/form-data">
      ...
      <div class="row g-3">
        ...
        <!-- Unidade/filial (apenas se houver mais de uma cadastrada) -->
        <?php if (count($unidades) > 1): ?>
        <div class="col-12">
          <label class="form-label fw-semibold" style="font-size:13px">
            <i class="bi bi-building me-1 text-primary"></i>Unidade / Filial
          </label>
          <?php
            $unid_sel = (int)($_POST['unidade_id'] ?? $unidade_padrao_id);
            // Mapa sla_multiplicador por unidade para o JS
            $unidades_json = json_encode(array_column($unidades, null, 'id'));
          ?>
          <select name="unidade_id" id="selectUnidade" class="form-select form-select-sm">
            <option value="">— Matriz / padrão —</option>
            <?php foreach ($unidades as $un): ?>
              <option value="<?= $un['id'] ?>"
                data-sla="<?= $un['sla_multiplicador'] ?>"
                <?= $unid_sel === (int)$un['id'] ? 'selected' : '' ?>>
                <?= h($un['nome']) ?><?= $un['codigo'] ? ' (' . h($un['codigo']) . ')' : '' ?>
                <?php if ((float)$un['sla_multiplicador'] !== 1.0): ?>
                  — SLA ×<?= number_format($un['sla_multiplicador'], 2) ?>
                <?php endif; ?>
              </option>
            <?php endforeach; ?>
          </select>
          <div id="slaHint" class="form-text" style="display:none"></div>
        </div>
        <?php endif; ?>
...
// Hint de multiplicador SLA por unidade
(function() {
  const sel = document.getElementById('selectUnidade');
  ...
  function updateHint() {
    ...
    if (opt?.value && sla !== 1.0) {
      hint.textContent = 'Prazo SLA multiplicado por ' + sla.toFixed(2) + ' para esta filial.';
      hint.style.display = '';
    } else {
