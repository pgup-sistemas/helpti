...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'confirmar') {
    ...
    foreach ($rows as $r) {
        ...
        $obs     = $r['observacoes'] ?? '';
        $ip_det  = trim($r['ip_detectado'] ?? '');
        $mac_det = strtolower(trim($r['mac'] ?? ''));
        $host    = trim($r['hostname'] ?? '');
        // Appenda hostname às observações se houver
        if ($host) $obs .= ($obs ? ' | ' : '') . 'Host: ' . $host;

        db()->prepare("
            INSERT INTO inventario (tipo,marca,modelo,numero_serie,patrimonio,setor,responsavel_nome,status,data_aquisicao,valor,garantia_ate,imei,observacoes,ip,mac_address,criado_em)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NOW())
        ")->execute([
            $r['tipo'],
            $r['marca'],
  ...
  <div class="col-md-5">
    <div class="card h-100">
      ...
      <div class="card-footer text-muted" style="font-size:11px">
        Colunas extras do scanner: <code>ip_detectado</code> e <code>mac</code> são salvas nos campos IP e MAC do equipamento; <code>hostname</code> vai para as observações.
      </div>
    </div>
  </div>
</div>
