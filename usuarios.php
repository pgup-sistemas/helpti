...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'criar') {
        ...
        // Soft-delete: preserva histórico de chamados e auditorias
        $pdo->prepare("UPDATE usuarios SET deleted_at = NOW(), ativo = 0 WHERE id=?")
            ->execute([$uid]);
        auditLog('usuario_desativado', 'usuarios', $uid, $nome);
