...
requireLogin();
$u_imp = usuario();
$pode_ver_cpf = in_array($u_imp['perfil'] ?? '', ['admin', 'gestora']);
...
<div class="secao">
  ...
  <div class="grid grid-3">
    ...
    <div class="campo"><label>CPF</label><span><?php
        $cpf_raw = $termo['responsavel_cpf'] ?: '';
        if (!$cpf_raw) { echo '—'; }
        elseif ($pode_ver_cpf) { echo h($cpf_raw); }
        else { echo preg_replace('/^(\d{3})\.(\d{3})\.(\d{3})-(\d{2})$/', '***.***.$3-**', $cpf_raw) ?: '***'; }
    ?></span></div>
    <div class="campo"><label>Matrícula</label><span><?php
        $mat_raw = $termo['responsavel_matricula'] ?: '';
        if (!$mat_raw) { echo '—'; }
        elseif ($pode_ver_cpf) { echo h($mat_raw); }
        else { $ml = mb_strlen($mat_raw); echo $ml <= 2 ? str_repeat('*', $ml) : h(mb_substr($mat_raw,0,1)).str_repeat('*',$ml-1); }
    ?></span></div>
...
<div class="assinaturas">
  <div class="ass-box">
    ...
    <?php if ($termo['responsavel_cpf']): ?>
    <div class="cargo">CPF: <?php
        if ($pode_ver_cpf) { echo h($termo['responsavel_cpf']); }
        else { echo preg_replace('/^(\d{3})\.(\d{3})\.(\d{3})-(\d{2})$/', '***.***.$3-**', $termo['responsavel_cpf']) ?: '***'; }
    ?></div>
    <?php endif; ?>
  </div>
  <div class="ass-box">
