...
SELECT t.id,
       IF(t.estoque_atual - COALESCE(saldo.v, 0) >= 0, 'entrada', 'saida'),
       ABS(t.estoque_atual - COALESCE(saldo.v, 0)),
       'Ajuste de reconciliacao inicial (migration 0004)',
       NULL, NOW()
FROM tipos_suprimentos t
LEFT JOIN (
    SELECT m.tipo_suprimento_id,
           SUM(CASE m.tipo WHEN 'entrada' THEN m.quantidade
                           WHEN 'saida'   THEN -m.quantidade
                           WHEN 'ajuste'  THEN m.quantidade
                           ELSE 0 END) AS v
    FROM estoque_movimentos m
    GROUP BY m.tipo_suprimento_id
) saldo ON saldo.tipo_suprimento_id = t.id
WHERE t.estoque_atual <> COALESCE(saldo.v, 0);
