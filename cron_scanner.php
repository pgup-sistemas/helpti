...
$json_file = __DIR__ . '/storage/scans/scan_ultimo.json';
...
// ── Sincroniza tabela impressoras a partir dos hosts descobertos ──────────────
// Hosts com tipo Impressora* que têm IP são inseridos/atualizados na tabela impressoras.
// Nunca sobrescreve dados já preenchidos manualmente (modelo_toner, numero_serie, setor).
$imp_novos = 0;
$imp_atualizados = 0;

$hosts_impressoras = $pdo->query("
    SELECT ip, mac_address, hostname, marca, tipo, setor, inventario_id
    FROM hosts_rede
    WHERE tipo LIKE '%Impressora%'
      AND ip IS NOT NULL AND ip != ''
      AND online = 1
")->fetchAll(PDO::FETCH_ASSOC);

foreach ($hosts_impressoras as $h) {
    $ip          = $h['ip'];
    $nome        = $h['hostname'] ?: ($h['marca'] ? $h['marca'] . ' ' . $ip : 'Impressora ' . $ip);
    $marca_model = $h['marca'] ?: '';
    $setor       = $h['setor'] ?: '';
    $inv_id      = $h['inventario_id'] ?: null;

    $existe = $pdo->prepare("SELECT id, setor, modelo_toner, numero_serie FROM impressoras WHERE ip = ?");
    $existe->execute([$ip]);
    $imp = $existe->fetch();

    if ($imp) {
        // Atualiza apenas campos que ainda estão vazios (não sobrescreve edições manuais)
        $pdo->prepare("
            UPDATE impressoras SET
                nome        = ?,
                marca_modelo = CASE WHEN (marca_modelo IS NULL OR marca_modelo = '') THEN ? ELSE marca_modelo END,
                setor        = CASE WHEN (setor IS NULL OR setor = '') THEN ? ELSE setor END,
                inventario_id = CASE WHEN inventario_id IS NULL THEN ? ELSE inventario_id END,
                status       = CASE WHEN status = 'Inativa' THEN 'Ativa' ELSE status END
            WHERE ip = ?
        ")->execute([$nome, $marca_model, $setor, $inv_id, $ip]);
        $imp_atualizados++;
    } else {
        $pdo->prepare("
            INSERT INTO impressoras (nome, marca_modelo, ip, setor, inventario_id, status)
            VALUES (?, ?, ?, ?, ?, 'Ativa')
        ")->execute([$nome, $marca_model, $ip, $setor, $inv_id]);
        $imp_novos++;
    }
}

// Marca como Inativa impressoras que não aparecem mais na rede (online=0 ou tipo mudou)
$pdo->exec("
    UPDATE impressoras imp
    LEFT JOIN hosts_rede h ON h.ip = imp.ip AND h.tipo LIKE '%Impressora%' AND h.online = 1
    SET imp.status = 'Inativa'
    WHERE h.id IS NULL AND imp.status = 'Ativa'
");
...
echo "  Impressoras    : {$imp_novos} novas, {$imp_atualizados} atualizadas\n";

cron_finish('scanner', true, "novos={$novos} atualizados={$atualizados} offline={$offline} imp_novos={$imp_novos}");
