...
$resumo = $pdo->query("SELECT
    ...
    FROM inventario WHERE deleted_at IS NULL")->fetch();

$por_tipo = $pdo->query("SELECT tipo, COUNT(*) AS total
    FROM inventario WHERE status != 'Descartado' AND deleted_at IS NULL
    GROUP BY tipo ORDER BY total DESC")->fetchAll();

$por_setor = $pdo->query("SELECT setor, COUNT(*) AS total
    FROM inventario WHERE status != 'Descartado' AND deleted_at IS NULL AND setor IS NOT NULL AND setor != ''
...
$garantias = $pdo->query("SELECT tipo, marca, modelo, patrimonio, setor, garantia_ate
    FROM inventario WHERE deleted_at IS NULL AND garantia_ate IS NOT NULL AND garantia_ate BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 90 DAY)
...
if ($fmt === 'csv' || $fmt === 'xlsx') {
    ...
    $dados = [$cab];
    $todos = $pdo->query("SELECT tipo,marca,modelo,numero_serie,patrimonio,setor,responsavel_nome,status,data_aquisicao,valor,garantia_ate FROM inventario WHERE deleted_at IS NULL ORDER BY setor, tipo")->fetchAll();
    foreach ($todos as $r) {
