...
$u_termos     = usuario();
$pode_ver_pii = in_array($u_termos['perfil'] ?? '', ['admin', 'gestora']);
    ...
    <div class="d-flex align-items-start justify-content-between px-4 py-3 border-bottom" style="gap:12px">
      <div style="min-width:0;flex:1">
        ...
        <div class="text-muted" style="font-size:12px;margin-top:2px">
          ...
          <?php if ($t['responsavel_matricula']): ?>
            · Matrícula: <?php if ($pode_ver_pii): ?>
              <?= h($t['responsavel_matricula']) ?>
            <?php else: $len = mb_strlen($t['responsavel_matricula']); ?>
              <?= $len <= 2 ? str_repeat('*', $len) : h(mb_substr($t['responsavel_matricula'],0,1)) . str_repeat('*', $len-1) ?>
            <?php endif; ?>
          <?php endif; ?>
