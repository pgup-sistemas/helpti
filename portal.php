...
// Unidades para o formulário de chamado
$unidades_portal = $pdo->query("SELECT id, nome, codigo, sla_multiplicador FROM unidades WHERE ativo=1 ORDER BY nome")->fetchAll();
$multi_unidade_portal = count($unidades_portal) > 1;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['_form'] ?? '') === 'abrir_chamado') {
    ...
    if (!$chamado_erros) {
        ...
        $unid_id_post = (int)($_POST['unidade_id'] ?? 0) ?: null;
        $pdo->prepare("INSERT INTO chamados
                (numero,descricao,setor,solicitante,telefone_solicitante,semana,status,origem,imagens,acompanhamento_token,avaliacao_token,unidade_id)
             VALUES (?,?,?,?,?,?,'Aberto','Formulário Web',?,?,?,?)")
            ->execute([$numero,$desc,$setor,$nome,$telefone?:null,$semana,$imagens_json,$acompToken,$avalToken,$unid_id_post]);
...
// helpers de badge inline — usam classes do design-tokens.css
function sbStatus(string $s): string {
    $m=['Aberto'=>'ht-badge-info','Em Andamento'=>'ht-badge-warn','Pendente'=>'ht-badge-danger','Concluído'=>'ht-badge-success'];
    return '<span class="ht-badge '.($m[$s]??'ht-badge-neutral').'">'.h($s).'</span>';
}
function sbNivel(string $n): string {
    if (str_contains($n,'Baixa')) return '<span class="ht-badge ht-badge-success">Baixa</span>';
    if (str_contains($n,'Média')) return '<span class="ht-badge ht-badge-warn">Média</span>';
    if (str_contains($n,'Alta'))  return '<span class="ht-badge ht-badge-danger">Alta</span>';
    return '<span class="ht-badge ht-badge-neutral">A Definir</span>';
}
function sbSup(string $s): string {
    $m=['Pendente'=>'ht-badge-warn','Aprovado'=>'ht-badge-info','Entregue'=>'ht-badge-success','Cancelado'=>'ht-badge-neutral'];
    return '<span class="ht-badge '.($m[$s]??'ht-badge-neutral').'">'.h($s).'</span>';
}
?>
<!DOCTYPE html>
...
<style>
/* ── Portal público — estilos exclusivos (tokens de cor e badges vêm do design-tokens.css) ── */
*{box-sizing:border-box}
body{background:var(--bg-page);font-family:'Manrope',system-ui,sans-serif;min-height:100vh;padding:0;margin:0;color:var(--tx-primary)}
h1,h2,h3{font-family:'Manrope',system-ui,sans-serif;letter-spacing:-.01em}
a{color:var(--brand)}
...
.portal-footer a{color:var(--tx-faint);font-size:12px;text-decoration:none;display:inline-flex;align-items:center;gap:5px;transition:.15s;margin:0 .6rem}
.portal-footer a:hover{color:var(--brand)}
.portal-footer-vendor{font-size:10px;color:var(--tx-faint);opacity:.7;margin-top:.5rem}

/* ── Container principal ── */
.portal-body{max-width:660px;margin:0 auto;padding:1.5rem 1rem 3rem}

/* ── Card com tabs integradas ── */
.portal-card{background:var(--bg-surface);border-radius:var(--radius-xl);box-shadow:var(--shadow-md);overflow:hidden;margin-bottom:1.25rem}

/* Abas de contexto */
.card-ctx-tabs{display:flex;border-bottom:1px solid var(--border)}
.card-ctx-tab{flex:1;padding:.65rem .5rem;text-align:center;font-size:13px;font-weight:600;color:var(--tx-muted);text-decoration:none;display:flex;align-items:center;justify-content:center;gap:5px;transition:.15s;border-bottom:2.5px solid transparent}
.card-ctx-tab:hover{color:var(--brand);background:var(--bg-surface-alt)}
.card-ctx-tab.active{color:var(--brand);border-bottom-color:var(--brand);background:var(--bg-surface-alt)}
.card-ctx-tab i{font-size:14px}

/* Sub-tabs (pills) */
.card-sub-tabs{display:flex;gap:6px;padding:.65rem 1.25rem;background:var(--bg-surface-alt);border-bottom:1px solid var(--border)}
.card-sub-tab{font-size:12px;font-weight:600;padding:.28rem .8rem;border-radius:var(--radius-pill);border:1.5px solid var(--border);color:var(--tx-muted);text-decoration:none;transition:.15s;display:flex;align-items:center;gap:4px}
...
/* Cabeçalho de formulário com gradiente */
.form-card-header{background:linear-gradient(135deg,var(--brand),var(--brand-mid));color:#fff;padding:1.25rem 1.5rem 1.1rem;text-align:center}
.form-card-header h2{font-size:16px;font-weight:800;margin:.5rem 0 0;color:#fff}
.form-card-header p{font-size:12px;opacity:.72;margin:.25rem 0 0}
.form-card-header .ico{width:40px;height:40px;border-radius:50%;background:rgba(255,255,255,.16);display:flex;align-items:center;justify-content:center;font-size:19px;margin:0 auto}
.form-card-body{padding:1.25rem}

/* Cards internos */
.form-card{background:var(--bg-surface);overflow:hidden;margin-bottom:0}
.panel-card{background:var(--bg-surface);overflow:hidden;margin-bottom:0}
.panel-card + .panel-card{border-top:1px solid var(--border)}

/* Inputs */
.form-label{font-weight:600;font-size:12.5px;color:var(--tx-secondary);margin-bottom:.25rem}
.form-control,.form-select{
  border-radius:var(--radius-md);border:1px solid var(--border);font-size:13.5px;
  padding:.5rem .75rem;background:var(--bg-surface);color:var(--tx-primary);transition:.15s;height:auto
}
.form-control:focus,.form-select:focus{border-color:var(--brand-mid);box-shadow:0 0 0 3px rgba(69,123,157,.10);background:var(--bg-surface)}
.form-control::placeholder{color:var(--tx-faint)}
/* Espaçamento compacto entre campos */
.form-card-body .mb-3{margin-bottom:.75rem!important}

/* Upload dropzone */
.upload-zone{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:.15rem;border:1.5px dashed var(--border);border-radius:var(--radius-lg);padding:1.35rem 1rem;cursor:pointer;transition:.2s;text-align:center;background:var(--bg-surface-alt)}
.upload-zone:hover{border-color:var(--brand-mid);background:var(--bg-hover)}
.upload-zone-icon{font-size:24px;color:var(--brand-mid);margin-bottom:.15rem}
.upload-zone-text{font-size:13px;font-weight:700;color:var(--tx-primary)}
.upload-zone-hint{font-size:11px;color:var(--tx-muted)}
.file-list{display:flex;flex-wrap:wrap;gap:6px;margin-top:.65rem}
.file-chip{display:flex;align-items:center;gap:6px;background:var(--sem-info-bg);border:1px solid var(--sem-info-bdr);color:var(--sem-info-tx);border-radius:var(--radius-pill);padding:.28rem .8rem .28rem .6rem;font-size:11.5px;font-weight:600;max-width:100%}
...
/* Botão IA */
.btn-ia{background:linear-gradient(135deg,#eef2ff,#f0f9ff);border:1.5px solid #c7d2fe;color:#4338ca;border-radius:var(--radius-pill);font-size:12px;font-weight:700;padding:.4rem 1rem;display:inline-flex;align-items:center;gap:6px;transition:.15s;cursor:pointer}
...
.btn-send:hover{background:var(--brand-mid)}
.btn-send:active{transform:scale(.98)}

/* Botão enviar */
.btn-send{width:100%;padding:.8rem;background:var(--brand);border:none;border-radius:var(--radius-md);color:#fff;font-size:14.5px;font-weight:700;cursor:pointer;transition:.15s;margin-top:.5rem}
.btn-send:hover{background:var(--brand-mid)}
.btn-send:active{transform:scale(.98)}

/* Botão outline brand */
.btn-outline-brand{border:1.5px solid var(--brand);color:var(--brand);background:transparent;border-radius:var(--radius-md);padding:.5rem 1.25rem;font-size:13px;font-weight:600;text-decoration:none;cursor:pointer;display:inline-block;transition:.15s}
.btn-outline-brand:hover{background:var(--brand);color:#fff}

/* Caixa de sucesso */
.success-box{text-align:center;padding:2rem 1.5rem}
.success-box .chk{font-size:52px;color:var(--sem-success-tx)}
.success-box h3{font-size:17px;font-weight:700;margin:.6rem 0 .3rem;color:var(--tx-primary)}
.success-box .num{display:inline-block;background:var(--sem-success-bg);border:1.5px solid var(--sem-success-bdr);color:var(--sem-success-tx);font-size:20px;font-weight:800;border-radius:var(--radius-md);padding:.45rem 1.25rem;letter-spacing:.05em;margin:.6rem 0}
.success-box p{font-size:13px;color:var(--tx-muted)}
.success-box .ss-hint{font-size:11.5px;color:var(--tx-faint);margin:.15rem 0 0}
.success-box .ss-link-sec{font-size:12.5px;color:var(--tx-muted);text-decoration:none;margin-top:.35rem}
...
.meus-itens-label{font-size:11px;font-weight:700;color:var(--tx-muted);text-transform:uppercase;letter-spacing:.05em;margin:0 0 .5rem 2px}
.meus-itens-lista{background:var(--bg-surface);border:1px solid var(--border);border-radius:var(--radius-lg);overflow:hidden}
.meu-item{display:flex;align-items:center;gap:6px;border-bottom:1px solid var(--border-light);transition:.12s}
.meu-item:last-child{border-bottom:none}
.meu-item:hover{background:var(--bg-hover)}
...
.meu-item .mi-data{font-size:11px;color:var(--tx-faint);white-space:nowrap}
.meu-item .mi-go{margin-left:auto;color:var(--tx-faint);font-size:12px}
.meu-item:hover .mi-go{color:var(--brand-mid)}
.meu-item .mi-esquecer{background:none;border:none;color:var(--tx-faint);font-size:12px;cursor:pointer;padding:6px 10px 6px 4px;line-height:1;flex-shrink:0}
.meu-item .mi-esquecer:hover{color:var(--sem-danger-tx)}

/* Painel acompanhamento */
.panel-card{background:var(--bg-surface);border-radius:var(--radius-xl);box-shadow:var(--shadow-md);overflow:hidden;margin-bottom:1rem}
...
.tl-dot{position:absolute;left:-25px;top:4px;width:11px;height:11px;border-radius:50%;background:var(--brand-mid);border:2px solid var(--bg-surface);box-shadow:0 0 0 2px var(--brand-mid)}
.tl-dot.sys{background:var(--tx-muted);box-shadow:0 0 0 2px var(--tx-muted)}
.tl-time{font-size:11px;color:var(--tx-faint);margin-bottom:2px}
.tl-text{font-size:13px;color:var(--tx-primary)}

/* Progress tracker */
.st-track{display:flex;justify-content:space-between;position:relative;margin-bottom:2rem;padding:0 .75rem}
.st-track::before{content:"";position:absolute;top:14px;left:10%;right:10%;height:4px;background:var(--border);z-index:1}
.st-prog{position:absolute;top:14px;left:10%;height:4px;background:var(--brand-mid);z-index:2;transition:width .3s}
.st-step{position:relative;z-index:3;text-align:center;width:30%}
.st-ico{width:32px;height:32px;border-radius:50%;background:var(--bg-surface);border:3px solid var(--border);color:var(--tx-muted);display:flex;align-items:center;justify-content:center;margin:0 auto 6px;transition:.2s}
.st-step.active .st-ico{border-color:var(--brand-mid);background:var(--brand-mid);color:#fff}
.st-step.current .st-ico{border-color:var(--brand);background:var(--brand);color:#fff;box-shadow:0 0 0 4px rgba(29,53,87,.15)}
.st-lbl{font-size:11px;font-weight:600;color:var(--tx-muted)}
.st-step.active .st-lbl,.st-step.current .st-lbl{color:var(--tx-primary)}

/* Carrinho suprimentos */
.cart-box{background:var(--bg-surface-alt);border:1px solid var(--border);border-radius:var(--radius-md);padding:.5rem 1rem;margin-bottom:1rem}
.item-row{display:flex;flex-direction:column;padding:.65rem 0;border-bottom:1px dashed var(--border)}
.item-row:last-child{border-bottom:none}
.btn-remove-item{background:var(--sem-danger-bg);color:var(--sem-danger-tx);border:none;border-radius:var(--radius-sm);width:36px;height:36px;display:flex;align-items:center;justify-content:center;transition:.15s;flex-shrink:0}
.btn-remove-item:hover{opacity:.8}
.btn-add-item{background:transparent;border:1.5px dashed var(--brand);color:var(--brand);border-radius:var(--radius-md);padding:.6rem;width:100%;font-weight:600;font-size:13px;transition:.2s}
  ...
  <?php else: ?>
    ...
    <div class="form-card-body">
      ...
      <form method="post" enctype="multipart/form-data" novalidate>
        ...
        <?php if ($multi_unidade_portal): ?>
        <div class="mb-3" id="div-unidade-portal">
          <label class="form-label">Unidade / Filial</label>
          <select name="unidade_id" id="unidade_id_portal" class="form-select">
            <option value="">— Selecione sua unidade —</option>
            <?php foreach ($unidades_portal as $un): ?>
              <option value="<?= $un['id'] ?>"
                data-sla="<?= h($un['sla_multiplicador']) ?>"
                <?= (($_POST['unidade_id'] ?? '') == $un['id']) ? 'selected' : '' ?>>
                <?= h($un['nome']) ?><?= $un['codigo'] ? ' (' . h($un['codigo']) . ')' : '' ?>
              </option>
            <?php endforeach; ?>
          </select>
          <div id="sla-hint-portal" class="text-warning mt-1" style="font-size:12px;display:none"></div>
        </div>
        <?php endif; ?>
        <div class="mb-3">
...
// ── Unidade / SLA hint (portal) ──
(function() {
  const sel  = document.getElementById('unidade_id_portal');
  const hint = document.getElementById('sla-hint-portal');
  if (!sel || !hint) return;
  function update() {
    const opt = sel.options[sel.selectedIndex];
    const sla = parseFloat(opt?.dataset?.sla ?? '1');
    if (opt?.value && sla !== 1.0) {
      hint.textContent = '⚠ Prazo SLA multiplicado por ' + sla.toFixed(2) + ' para esta unidade.';
