...
$garantias_alerta = $pdo->query("
    ...
    WHERE deleted_at IS NULL
      AND garantia_ate BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 60 DAY)
...
<div class="card mb-4">
  ...
  <div class="card-body" style="height:240px"><canvas id="chartDash"></canvas></div>
</div>

<!-- ── Chamados abertos — largura total ── -->
<div class="card mb-4">
    ...
    <table class="table table-hover mb-0">
      ...
      <tbody>
        ...
        <tr data-href="chamado.php?id=<?= $c['id'] ?>">
          <td style="white-space:nowrap"><code style="font-size:12px"><?= h($c['numero']) ?></code></td>
          <td style="max-width:340px"><div style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="<?= h($c['descricao']) ?>"><?= h($c['descricao']) ?></div></td>
...
<!-- ── Widgets em linha (3 colunas) ── -->
<?php if (!empty($meusChamados) || $sup_pendentes || $termos_alerta || $contratos_alerta || $garantias_alerta): ?>
<div class="row g-4">
