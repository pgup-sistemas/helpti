...
$c = $pdo->prepare("SELECT c.*, u.nome AS resp_nome,
    un.nome AS unidade_nome, un.codigo AS unidade_codigo, un.sla_multiplicador
    FROM chamados c
    ...
    <div class="card mb-3">
      ...
      <div class="card-body">
        ...
        <div class="row g-3">
          ...
          <?php if ($chamado['unidade_nome']): ?>
          <div class="col-sm-6">
            <div class="field-label" style="font-size:12px">Unidade / Filial</div>
            <div class="fw-semibold">
              <i class="bi bi-building me-1 text-primary"></i><?= h($chamado['unidade_nome']) ?>
              <?php if ($chamado['unidade_codigo']): ?>
                <span class="text-muted fw-normal" style="font-size:12px">(<?= h($chamado['unidade_codigo']) ?>)</span>
              <?php endif; ?>
              <?php if ((float)$chamado['sla_multiplicador'] !== 1.0): ?>
                <span class="badge bg-warning text-dark ms-1" style="font-size:10px">SLA ×<?= number_format($chamado['sla_multiplicador'],2) ?></span>
              <?php endif; ?>
            </div>
          </div>
          <?php endif; ?>
