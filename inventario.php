...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'criar') {
        ...
        // Captura estado anterior para log de movimentação
        $anterior = $pdo->prepare("SELECT setor, responsavel_nome, status FROM inventario WHERE id=?");
        $anterior->execute([$id]);
        ...
        // Registra movimentação se houve mudança em setor, responsável ou status
        if ($anterior) {
            $mudou_setor  = $anterior['setor']            !== $campos['setor'];
            $mudou_resp   = $anterior['responsavel_nome'] !== $campos['responsavel_nome'];
            $mudou_status = $anterior['status']           !== $campos['status'];
            if ($mudou_setor || $mudou_resp || $mudou_status) {
                $tipo_mov = $campos['status'] === 'Descartado' ? 'Descarte'
                          : ($campos['status'] === 'Em Manutenção' ? 'Manutenção' : 'Transferência');
                try {
                    $pdo->prepare("INSERT INTO historico_movimentacao
                        (inventario_id, usuario_id, tipo, setor_anterior, setor_novo, resp_anterior, resp_novo, status_anterior, status_novo)
                        VALUES (?,?,?,?,?,?,?,?,?)")
                        ->execute([$id, usuario()['id'] ?? null, $tipo_mov,
                            $anterior['setor'], $campos['setor'],
                            $anterior['responsavel_nome'], $campos['responsavel_nome'],
                            $anterior['status'], $campos['status']]);
                } catch (Throwable) { /* historico_movimentacao pode não existir ainda */ }
            }
        }
        ...
        $termos = $pdo->prepare("SELECT COUNT(*) FROM termos_uso WHERE inventario_id=? AND status='Ativo'");
        $termos->execute([$id]);
        if ($termos->fetchColumn() > 0) {
            flash('Equipamento possui termos de uso ativos. Devolva os termos antes de desativar.', 'danger');
        } else {
            $pdo->prepare("UPDATE inventario SET deleted_at = NOW() WHERE id=? AND deleted_at IS NULL")
                ->execute([$id]);
            auditLog('inventario_desativado', 'inventario', $id);
            flash('Equipamento desativado do inventário ativo.');
        }
        ...
        if ($com_termo->fetchColumn() > 0) {
            flash('Alguns equipamentos possuem termos de uso ativos e não podem ser desativados.', 'danger');
        } else {
            $pdo->prepare("UPDATE inventario SET deleted_at = NOW() WHERE id IN ($placeholders) AND deleted_at IS NULL")
                ->execute($ids);
            auditLog('inventario_desativado_massa', 'inventario', 0, count($ids).' ativos');
            flash(count($ids).' equipamento(s) desativado(s).');
        }
    }
    header('Location: inventario.php'); exit;
}
...
// Stats
$stats_inv = $pdo->query("SELECT status, COUNT(*) n FROM inventario WHERE deleted_at IS NULL GROUP BY status")->fetchAll(PDO::FETCH_KEY_PAIR);
$garantias = $pdo->query("SELECT COUNT(*) FROM inventario WHERE deleted_at IS NULL AND garantia_ate IS NOT NULL AND garantia_ate BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 60 DAY)")->fetchColumn();
...
try {
    $disp_online = (int)$pdo->query("
        ...
        JOIN hosts_rede h ON h.inventario_id = i.id
        WHERE h.online = 1 AND i.status = 'Disponível' AND i.deleted_at IS NULL
    ")->fetchColumn();
