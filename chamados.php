...
if ($filtros['unidade'] === '0') { $where[] = "c.unidade_id IS NULL"; }
elseif ($filtros['unidade'])     { $where[] = "c.unidade_id=:unidade"; $params['unidade'] = $filtros['unidade']; }
...
$sql = "SELECT c.*, u.nome AS resp_nome, un.nome AS unidade_nome, un.codigo AS unidade_codigo
        FROM chamados c
        LEFT JOIN usuarios u ON u.id=c.responsavel_id
...
$unidades_lista = $pdo->query("SELECT id,nome,codigo FROM unidades WHERE ativo=1 ORDER BY nome")->fetchAll();
$multi_unidade  = count($unidades_lista) > 1;
...
<div class="card mb-3">
  <div class="card-body py-3">
    <form method="get" class="row g-2 align-items-end">
      ...
      <?php if ($multi_unidade): ?>
      <div class="col-md-2">
        <label class="form-label fw-semibold" style="font-size:12px">Unidade</label>
        <select name="unidade" class="form-select form-select-sm">
          <option value="">Todas</option>
          <option value="0" <?= $filtros['unidade']==='0'?'selected':'' ?>>Sem unidade</option>
          <?php foreach ($unidades_lista as $un): ?>
            <option value="<?= $un['id'] ?>" <?= $filtros['unidade']==$un['id']?'selected':'' ?>>
              <?= h($un['nome']) ?><?= $un['codigo'] ? ' ('.$un['codigo'].')' : '' ?>
            </option>
          <?php endforeach; ?>
        </select>
      </div>
...
<div class="card">
  ...
  <div class="table-responsive">
    <table class="table table-hover mb-0 table-sortable">
      <thead>
        <tr>
          <th data-sort>Nº</th><th data-sort>Descrição</th><th data-sort>Setor</th><?php if ($multi_unidade): ?><th data-sort>Unidade</th><?php endif; ?><th data-sort>Solicitante</th>
      ...
      <tbody>
        ...
        <tr data-href="chamado.php?id=<?= $c['id'] ?>">
          ...
          <td style="font-size:12px">
            <?php if ($c['unidade_nome']): ?>
              <i class="bi bi-building me-1 text-primary" style="font-size:11px"></i><?= h($c['unidade_codigo'] ?: $c['unidade_nome']) ?>
            <?php else: ?>
