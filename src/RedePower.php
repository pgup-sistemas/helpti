...
final class RedePower
{
    ...
    public static function executar(string $acao, string $ip, int $segundos = 30, string $mensagem = ''): array
    {
        ...
        if (PHP_OS_FAMILY === 'Windows') {
            return self::executarWindows($acao, $ip, $segundos, $mensagem);
        }
        ...
        // 2ª tentativa: conta local (WORKGROUP).
    ...
    /**
     * Desligar/reiniciar/cancelar via comandos nativos do Windows (shutdown.exe).
     * Não precisa de Samba nem credenciais especiais — usa as do usuário atual do serviço
     * Apache (SYSTEM ou conta configurada). Para autenticação explícita use SHUTDOWN_USER/PASS.
     * @return array{ok:bool, saida:string}
     */
    private static function executarWindows(string $acao, string $ip, int $segundos, string $mensagem): array
    {
        if ($acao === 'cancelar') {
            $cmd = 'shutdown /a /m \\\\' . escapeshellarg($ip) . ' 2>&1';
            $saida = trim((string) shell_exec($cmd));
            $ok = $saida === '' || stripos($saida, 'successfully') !== false
               || stripos($saida, 'aborted') !== false
               || stripos($saida, 'nenhum') !== false
               || stripos($saida, '1190') !== false; // "No pending shutdown"
            return ['ok' => $ok, 'saida' => $saida ?: 'Desligamento cancelado.'];
        }

        $partes = ['shutdown'];
        $partes[] = $acao === 'reiniciar' ? '/r' : '/s';
        $partes[] = '/m';
        $partes[] = '\\\\' . $ip;
        $partes[] = '/t';
        $partes[] = (string) $segundos;
        $partes[] = '/f';
        $msg = mb_substr(trim($mensagem), 0, 240);
        if ($msg !== '') {
            $partes[] = '/c';
            $partes[] = $msg;
        }
        ...
        $saida = trim((string) shell_exec($cmd));
        $ok = $saida === ''
           || stripos($saida, 'successfully') !== false
           || stripos($saida, '5') === false; // exit 5 = access denied
        // Saída vazia = sucesso no shutdown.exe do Windows
        if ($saida === '') {
            return ['ok' => true, 'saida' => 'Comando enviado com sucesso.'];
        }
    ...
    /** Nome NetBIOS da máquina. */
    private static function netbiosNome(string $ip): string
    {
        if (PHP_OS_FAMILY === 'Windows') {
            $out = (string) shell_exec('nbtstat -A ' . escapeshellarg($ip) . ' 2>nul');
            // nbtstat output: "   COMPUTERNAME    <20>  UNIQUE  Registered"
            if (preg_match('/^\s*([A-Za-z0-9_-]{1,15})\s+<20>/m', $out, $m)) {
                return strtoupper($m[1]);
            }
            if (preg_match('/^\s*([A-Za-z0-9_-]{1,15})\s+<00>\s+UNIQUE/m', $out, $m)) {
                return strtoupper($m[1]);
            }
            return '';
        }
