...
// LGPD: mascaramento de dados pessoais sensíveis
function maskCpf(?string $cpf): string {
    if (!$cpf || $cpf === '—') return '—';
    // Mantém apenas os 3 dígitos do meio visíveis: ***.456.***-**
    return preg_replace('/^(\d{3})\.(\d{3})\.(\d{3})-(\d{2})$/', '***.***.$3-**', $cpf) ?: '***';
}
function maskMatricula(?string $mat): string {
    if (!$mat || $mat === '—') return '—';
    $len = mb_strlen($mat);
    return $len <= 2 ? str_repeat('*', $len) : mb_substr($mat, 0, 1) . str_repeat('*', $len - 1);
}
...
$rows8 = $pdo->query("SELECT tipo,marca,modelo,numero_serie,patrimonio,setor,status,
    ...
    FROM inventario WHERE deleted_at IS NULL ORDER BY tipo,marca,modelo")->fetchAll();
...
// ── 10. Termos de uso (LGPD: CPF/Matrícula mascarados para não-admin) ─────────
...
$cpf_header  = $is_admin ? bold('CPF')       : bold('CPF (mascarado)');
$mat_header  = $is_admin ? bold('Matrícula') : bold('Matrícula (mascarada)');
$sheet10 = [[bold('ID'),bold('Tipo'),bold('Marca/Modelo'),bold('S/N'),bold('Patrimônio'),bold('Responsável'),$cpf_header,$mat_header,bold('Setor'),bold('Entrega'),bold('Prev. Devolução'),bold('Devolvido em'),bold('Status'),bold('Condição Entrega'),bold('Condição Devolução'),bold('Observações')]];
foreach ($rows10 as $r) {
    $cpf = $is_admin ? ($r['responsavel_cpf'] ?: '—') : maskCpf($r['responsavel_cpf']);
    $mat = $is_admin ? ($r['responsavel_matricula'] ?: '—') : maskMatricula($r['responsavel_matricula']);
    $sheet10[] = [$r['id'],$r['tipo'],$r['marca'].' '.$r['modelo'],$r['numero_serie']?:'—',$r['patrimonio']?:'—',
        $r['responsavel_nome'],$cpf,$mat,$r['setor'],
