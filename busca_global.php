...
if ($usaFulltext) {
    $rows = $pdo->prepare("
        ...
        FROM inventario
        WHERE deleted_at IS NULL
          AND (MATCH(tipo, marca, modelo, numero_serie, patrimonio, setor) AGAINST(:b IN BOOLEAN MODE)
    ...
    $rows = $pdo->prepare("
        ...
        FROM inventario
        WHERE deleted_at IS NULL
          AND (tipo LIKE :l OR marca LIKE :l2 OR modelo LIKE :l3
...
$rows = $pdo->prepare("SELECT id, nome, email, perfil FROM usuarios WHERE deleted_at IS NULL AND (nome LIKE ? OR email LIKE ?) LIMIT 4");
